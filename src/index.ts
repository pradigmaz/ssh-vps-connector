...
class SSHVPSConnector {
  ...
  private async loadVPSConfig(): Promise<VPSConfig | null> {
    try {
  ...
  private async saveVPSConfig(config: VPSConfig): Promise<void> {
    await this.ensureConfigDir();
    await fs.writeFile(this.vpsConfigPath, JSON.stringify(config, null, 2));
  }

  private async connectSSH(config: any): Promise<void> {
    const finalConfig = {
  ...
  private async collectDockerContainers(): Promise<any[]> {
    try {
  ...
  private async collectDockerComposeFiles(): Promise<any[]> {
    try {
  ...
  private async extractCredentials(containers: any[]): Promise<Record<string, any>> {
  ...
  private async collectNetworkConfig(): Promise<any> {
    try {
  ...
  private async collectVPSData(): Promise<VPSConfig> {
  ...
  private async initializeVPSData(): Promise<void> {
    try {
